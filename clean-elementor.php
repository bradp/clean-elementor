...
function add_options_filters() {
	...
	$options = apply_filters( 'clean_elementor_options', $options );
...
function add_user_meta_filters( $value, $object_id, $meta_key, $single ) {
	if ( ! apply_filters( 'clean_elementor_hide_admin_notices', true ) ) {
		return $value;
	}
...
function remove_settings_page() {
	if ( apply_filters( 'clean_elementor_remove_settings_page', true ) ) {
